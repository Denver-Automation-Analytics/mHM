...
class BaseGrid(object):
    ...
    def enlargeGrid(self, grid):
        ...
        if (
            bbox["ymin"] > self_bbox["ymin"]
            or bbox["ymax"] < self_bbox["ymax"]
            or bbox["xmin"] > self_bbox["xmin"]
            or bbox["xmax"] < self_bbox["xmax"]
        ):
...
class AsciiGrid(BaseGrid):
    ...
    def padGrid(self=0, top=0, left=0, bottom=0, right=0):
        ...
        data = [
            "{0} {1} {2}".format(left_pad, row, right_pad)
...
class NetcdfGrid(BaseGrid):
    ...
    def write(self, filename):
        ...
        dumpnetcdf(
            ...
            time=(self.nctime, self.nctimeatt),
        )
        ...
        dumpnetcdf(
            ...
            dims=["yc", "xc"],
            create=False,
            lon=(self.nclon, self.nclonatt),
            lat=(self.nclat, self.nclatatt),
        )
        ...
        dumpnetcdf(
            ...
            **{
                "dims": ["time", "yc", "xc"],
                "create": False,
                self.ncvar: (self.data, self.ncatt),
            },
        )
    ...
    def padGrid(self=0, top=0, left=0, bottom=0, right=0):
        ...
        # initialize 3d field
        self.data = (
            zeros((self.ncarr.shape[0], self.nrows, self.ncols)) + self.nodata_value
        )
        # paste 3d field
        print(
            "***CAUTION: grid cell at (0,0) coordinate is assumed to be in the North-West corner"
        )
        self.data[
            :, top : top + self.ncarr.shape[1], left : left + self.ncarr.shape[2]
        ] = self.ncarr
        # also extend lat and lon
        dummy = zeros((self.nrows, self.ncols)) + self.nodata_value
        dummy[top : top + self.ncarr.shape[1], left : left + self.ncarr.shape[2]] = (
            self.nclon
        )
        self.nclon = dummy
        dummy = zeros((self.nrows, self.ncols)) + self.nodata_value
...
if __name__ == "__main__":
    if (
        (len(sys.argv) < 4)
        or (len(sys.argv) > 5)
        or not all([os.path.isfile(a) for a in sys.argv[1:-1]])
    ):
