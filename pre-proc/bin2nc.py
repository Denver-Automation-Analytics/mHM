...
parser.add_option(
    ...
    type="string",
    default=indir,
    metavar="Directory of *.bin files.",
...
parser.add_option(
    ...
    "--varnamelong",
    action="store",
    dest="variable_long_name",
    type="string",
    default=variable_long_name,
    metavar="Long_name in NetCDF.",
...
varName = "time"
dims = None
varAtt = [["units", startTime], ["calendar", "standard"]]
thand = writenetcdf(
    fhandle, name=varName, dims=dims, attributes=varAtt, isdim=True, vartype="i4"
)
#
varAtt = [["axis", "X"]]
varName = "xc"
...
# Variables
# lon
varAtt = [["units", "degrees_east"], ["long_name", "longitude"]]
#
varName = "lon"
...
# lat
varAtt = [["units", "degrees_north"], ["long_name", "latitude"]]
varName = "lat"
var = readnetcdf(latlonfile, var=varName)
dims = ["yc", "xc"]
writenetcdf(fhandle, name=varName, dims=dims, var=var, attributes=varAtt, comp=True)
...
dims = ["time", "yc", "xc"]
vhandle = writenetcdf(
    fhandle, name=varName, dims=dims, attributes=varAtt, vartype="f8", comp=True
)
# -------------------------------------------------------------------------
# Read binary files
# -------------------------------------------------------------------------
