...
def header_to_latlon(headerfile, coord_sys, do_corners=False):
    ...
    if not os.path.isfile(headerfile):
        sys.exit(
            "No " + headerfile + " file found here, are you in the right directory?"
        )
...
def latlon_to_nc(fhandle, lons, lats, xx, yy, missVal, suffix):
    ...
    writenetcdf(
        fhandle, name=varName, dims=dims, var=var, attributes=varAtt, isdim=True
    )
    #
    varAtt = [["axis", "Y"]]
    varName = "yc" + suffix[0]
...
parser.add_option(
    ...
    dest="headerfile_l0",
    type="string",
    default=headerfile_l0,
    metavar="Header file for level 0",
...
parser.add_option(
    ...
    dest="headerfile_l1",
    type="string",
    default=headerfile_l1,
    metavar="Header file for level 1",
...
parser.add_option(
    ...
    dest="headerfile_l11",
    type="string",
    default=headerfile_l11,
    metavar="Header file for level 11",
...
if headerfile_l0 == "" and headerfile_l1 == "" and headerfile_l11 == "":
    raise ValueError(
        "***ERROR: no headerfile specified, use -h switch for more information"
    )
