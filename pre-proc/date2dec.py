    ...
    if (calendar == "standard") or (calendar == "gregorian"):
        ...
        if is121 and (nt.__version__ < "1.2.2"):
            for ii, tt in enumerate(timeobj):
                output[ii] = nt.date2num(tt, units, calendar="gregorian") + dec0
        else:
        ...
        if is121 and (nt.__version__ < "1.2.2"):
            for ii, tt in enumerate(timeobj):
                output[ii] = nt.date2num(tt, units, calendar="proleptic_gregorian")
        else:
        ...
        if doerr:
            output = np.where(output >= 60.0, output + 1.0, output)
            # date2num treats 29.02.1900 as 01.03.1990, i.e. is the same decimal number
            if np.any((output >= 61.0) & (output < 62.0)):
                for i in range(outsize):
        ...
        output = np.array(yr, dtype=np.float) + (
            (tdy - 1.0) * 24.0
            + np.array(hr, dtype=np.float)
            + np.array(mi, dtype=np.float) / 60.0
            + np.array(sc, dtype=np.float) / 3600.0
        ) / ((days_year + np.array(leap, dtype=np.float)) * 24.0)
