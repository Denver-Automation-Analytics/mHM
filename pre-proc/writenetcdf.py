        ...
        if time is not None:
            ...
            if np.size(np.shape(time)) == 0:
                if np.size(svar) != (np.size(shand) - 1):
                    raise ValueError(
                        "Variable and handle dimensions do not agree for variable time: "
                        + str(svar)
                        + " and "
                        + str(shand)
                    )
                ...
                if np.size(svar) != np.size(shand):
                    raise ValueError(
                        "Variable and handle dimensions do not agree for variable time vector: "
                        + str(svar)
                        + " and "
                        + str(shand)
                    )
            ...
            if np.size(shand) == 1:
                ...
                raise ValueError(
                    "Number of dimensions not supported (>6): " + str(np.size(shand))
                )
        else:
            if np.size(var) != np.size(hand):
                raise ValueError(
                    "Variable and handle elements do not agree: "
                    + str(np.size(var))
                    + " and "
                    + str(np.size(hand))
                )
