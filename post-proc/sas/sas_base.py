...
class SAS(MHM):
    def __init__(self, model_path, file_name="mhm.nml", rel_path_name=False):
    ...
    def get_p_rt(self):
        ...
        for t_i in range(0, self.t_no):
            ...
            self.soil_moisture_rel.data[t_i, :, :] = (
                soil_moisture[t_i, :, :] / soil_moisture_tot
            )
        ...
        for x_i in range(0, self.x_no):
            for y_i in range(0, self.y_no):
                if self.mask["L1_special"][y_i, x_i]:
                    continue
                for t_in in range(0, self.t_no):
                    self.theta[t_in, y_i, x_i] = get_theta(
                        S[:, y_i, x_i], Q_1[:, y_i, x_i], Q_2[:, y_i, x_i], self.t, t_in
                    )
                    self.eta[t_in, y_i, x_i] = get_theta(
                        S[:, y_i, x_i], Q_2[:, y_i, x_i], Q_1[:, y_i, x_i], self.t, t_in
                    )
    ...
    def fit_p(self, func="exp"):
        ...
        for x_i in range(0, self.x_no):
            for y_i in range(0, self.y_no):
                ...
                if func == "exp":
                    popt, pcov = curve_fit(
                        exp_dist, self.t[:100], self.mean_p_Q[:100, y_i, x_i]
                    )
                if func == "gam":
                    popt, pcov = curve_fit(
                        gamma_dist, self.t[:100], self.mean_p_Q[:100, y_i, x_i]
                    )
    ...
    def get_wet_dry_years(self, time_series):
        ...
        my_soil_moisture = (
            self.mean_soil_moisture - np.mean(self.mean_soil_moisture)
    ...
    def compress_flux(self, data_list, chunk_no):
        for data_i in data_list:
            setattr(self, data_i, self.sum_chunk(getattr(self, data_i), chunk_no, 0))

    def compress_state(self, data_list, chunk_no):
        for data_i in data_list:
            setattr(self, data_i, self.mean_chunk(getattr(self, data_i), chunk_no, 0))
