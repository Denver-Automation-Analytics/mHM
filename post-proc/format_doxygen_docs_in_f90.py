...
class Doc(object):
    ...
    INTENT_DICT = {
        ...
        "out": "INTENT(OUT)",
        "inout": "INTENT(INOUT)",
        "inoptional": "INTENT(IN), OPTIONAL",
        "outoptional": "INTENT(OUT), OPTIONAL",
    ...
    def parse_doc(self, line):
        if not check_for_line_to_ignore(line):
            ...
            if self._in_modified:
                ...
                re.match(self.COMMENT + "\s*[A-Z\(\),\s]+$", line.strip())
                or re.match(self.COMMENT + "\s*None\s*", line.strip())
            ):
    ...
    def doc_to_str(self, fname=None, indent=0, module=False):
        ...
        def format_item(indent, comment, block_name, value):
            ...
            if block_name is not None:
                formatted += (
                    DOXYGEN_TEMPLATE.format(
                        " " * indent, self.COMMENT, block_name, width=self.BEGIN_INDENT
                    )
                    + "\n"
                )
            sep = DOXYGEN_TEMPLATE.format(
                " " * indent, comment, "", width=self.SUBSEQUENT_INDENT
            )
            formatted += ("\n").join(
                [
                    ("\n" + sep).join(wrap(s, MAX_LINE_WIDTH, initial_indent=sep))
        ...
        if module:
            ...
            out_string += format_item(
                indent=indent, comment=self.COMMENT, block_name="NAME", value=value
            )
            for item in ["brief", "details"]:
                ...
                out_string += format_item(
                    ...
                    block_name=self.BLOCK_DICT.get(item),
                    value=value,
                )
            if self.param_dict:
                for param_key, items in self.param_dict.items():
                    ...
                    value = "\n".join(
                        [
                            "{:{width}} {}".format(*item, width=left_width)
                            for item in items
                        ]
                    )
                    out_string += format_item(
                        indent=indent,
                        comment=self.DOC_COMMENT,
                        block_name=self.INTENT_DICT[param_key],
                        value=value,
                    )
        ...
        if self._modified:
            ...
            for mod in self._modified:
                mod.note = "- " + mod.note.lstrip("- ").replace(
                    "- ",
                    "\n{}{}{}- ".format(
                        " " * (indent), self.COMMENT, " " * (11 + max_editor_length)
                    ),
                )
                try:
                    out_string += "{}{} {:{width}} {} {}\n".format(
...
class FortranFile(Doc):
                ...
                if not self._in_module and not line.strip().startswith("#"):
                    ...
                    if self._cur_doc is not None:
                        self.lines.extend(
                            [
                                _line + "\n"
                                for _line in self._cur_doc.doc_to_str(
                                    indent=2, module=True
                                ).split("\n")
                            ]
                        )
                        self._cur_doc = None
    ...
    def _check_for_file_doc(self, line):
        ...
        if any(
            [
                re.search(pattern, line.split(self.COMMENT)[0], re.IGNORECASE)
                for pattern in self.MODULE_FLAG
            ]
        ):
    ...
    def _check_for_routine(self, line):
        ...
        if any(
            [
                re.match(
                    "{} ".format(pattern),
                    line.split(self.COMMENT)[0].strip(),
                    re.IGNORECASE,
                )
                or re.match(
                    "{}$".format(pattern),
    ...
    def _check_for_doc(self, line):
        # check for DOC_COMMENT
        if re.match(self.DOC_COMMENT, line.strip()) or re.match(
            "!\s+NAME", line.strip()
        ):
            self._in_doc = True
    ...
    def _check_for_code(self, line):
        ...
        if (
            not (
                ARG_DESCRIPTION_SPLITTER in clean_line
                or re.match(USE_FLAG, clean_line, re.IGNORECASE)
                or re.match(IMPLICIT_NONE_FLAG, clean_line, re.IGNORECASE)
                or re.match("import ", clean_line, re.IGNORECASE)
            )
            and not self._is_line_continued
            and clean_line
        ):
            self._in_routine = False
            self._in_code = True
    ...
    def _parse_routine(self, line):
        ...
        if not line.strip().startswith("#"):
            self._in_routine_args = self._cur_routine.parse_line(
                line,
...
class Routine(object):
    ...
    def parse_line(
        self, line, is_line_continued=False, in_routine_args=False, in_flag=""
    ):
        # check for DOC_COMMENT
        do_parse = True
    ...
    def _parse_body(self, line, is_line_continued, in_flag):
        ...
        if matched:
            ...
            line = line.strip()[
                re.match("(!\$)?\s*" + USE_FLAG, line.strip(), re.IGNORECASE).end() :
            ]
    ...
    def _args_to_str(self, indent=0):
        args_str = "{}{}{} {}".format(
            " " * indent, self.type_attrs or "", self.type, self.name
        )
    ...
    def _use_to_str(self, indent=0):
        def format_use(use_item):
            if use_item[2] == "$":
                ...
                new_string = "{}use {}, {} {} {}".format(
                    " " * indent, use_item[0], self.ONLY_STR, ":", use_item[1]
                )
        ...
        if self.used:
            ...
            if prev_flag != "$":
                out_string += "\n{}".format(
                    self._flag_to_str(prev_flag, negative=False)
                )
            ...
            for i_item, use_item in enumerate(used[1:], 1):
                if use_item[2] != prev_flag and use_item[2] != "$":
                    ...
                    if prev_flag:
                        out_string += "\n{}".format(
                            self._flag_to_str("", negative=True)
                        )
                    # now write the new flag
                    prev_flag = use_item[2]
                    out_string += "\n{}".format(
                        self._flag_to_str(prev_flag, negative=True)
                    )
                # here the actual use statement is written
                if use_item[0] == used[i_item - 1][0]:
                    if (
                        arg_length + total_length + len(use_item[1]) + 2
                        > MAX_LINE_WIDTH
                    ):
    ...
    def _variables_to_str(self, indent):
        ...
        for attr in self.args_attrs:
            ...
            if attr.doc:
                sep = DOXYGEN_TEMPLATE.format(" " * indent, self.COMMENT, "", width=2)
                out_string += ("\n").join(
                    [
                        ("\n" + sep).join(wrap(s, MAX_LINE_WIDTH, initial_indent=sep))
                        for s in attr.doc.splitlines()
                    ]
                ) + "\n"
            out_string += "{}{} {} {}\n\n".format(
                " " * indent, ", ".join(attr.attrs), ARG_DESCRIPTION_SPLITTER, attr.name
            )
        return out_string + "\n"
    ...
    def update_doc(self, default_author):
        ...
        for arg, arg_doc in zip(self.args, self.args_doc):
            ...
            if arg_attrs.intent_optional_key:
                self.doc.param_dict.setdefault(
                    arg_attrs.intent_optional_key, []
                ).append(arg_attrs.doxygen_string)
    ...
    def _imports_to_str(self, indent):
        if self.imports:
            return (
                "".join(
                    [
                        "{}{}\n".format(" " * indent, cur_import)
                        for cur_import in self.imports
                    ]
                )
                + "\n"
            )
...
class ArgumentAttributes(object):
    ...
    def doxygen_string(self):
        param_args = [
            arg
            for arg in [self.type_attr, self.dimension_attr, self.optional_attr]
            if arg
        ]
        return (
            '\\param[{}] "{} {} {}"'.format(
...
class TestFortranFile(object):
    ...
    def test_check_for_routine(self, fortran_file):
        lines = [
            "function newDimAlias(aliases) result(newDimAlias)",
    ...
    def test_check_for_code(self, fortran_file):
        lines = ["    runoff_sealed = 0.0_dp", "    end do ! hh"]
        for line in lines:
