...
DOMAIN_BUFFER_M = (
    1000  # buffer [m] grown around the domain polygon when clipping/gridding inputs
)
L0_CELL_SIZE_M = (
    250  # Landscape detail grid resolution (mod10, mod13, mod14, mod15, mod16)
)
L1_CELL_SIZE_M = (
    1000  # Hydrologic simulation grid resolution (i.e., mHM output resolution)
)
...
START_DATE = (
    date.fromisoformat(EVAL_START_DATE) - timedelta(days=WARMUP_DAYS)
).isoformat()
TIMESTEP = "hourly"  # "hourly" or "daily"; used by mod11, mod12, mod15, mod19
ROUTING_METHOD = (
    "muskingum"  # mRM routing (mod19): "muskingum" | "adaptive" | "adaptive_varying"
)
OPTI_OBJECTIVE = "nse_lnnse"  # mod19 calibration objective: "nse" | "lnnse" | "nse_lnnse" | "kge" | "multi_kge" | "wnse" | "kge_q_et"
N_ITERATIONS = (
    1000  # mod19 DDS optimizer trials; more = better calibration, longer runtime
)
SEED = 32  # mod19 DDS random seed; -9 = clock-based (nondeterministic). Set a positive int for reproducible A/B runs.
N_OMP_THREADS = (
    25  # OpenMP threads for mHM; requires binary built with -DCMAKE_WITH_OpenMP=ON
)
...
TRITON_DOMAIN_NAME = (
    "frio_river"  # basename for the TRITON files (<name>.dem, <name>.roff, ...)
)
TRITON_DEM_CELLSIZE_M = 10  # TRITON grid resolution [m]; mod10 dem_corrected.tif is reprojected/resampled to this
TRITON_PROJECTION = (
    OUTPUT_CRS  # projected CRS written to the TRITON .cfg (must match the runoff grid)
)
...
TRITON_CONST_MANN = (
    0.060  # fallback constant Manning roughness used where land cover is unavailable
)
...
TRITON_IO_LULC_YEAR = (
    None  # IO annual mosaic year (e.g. "2023"); None = latest available
)
...
TRITON_END_DATE = (
    "2026-07-25"  # event-window end 'YYYY-MM-DD' (inclusive); None = full mHM record
)
...
TRITON_BF_SLOPE_MIN = (
    1e-4  # floor on bed slope [m/m] in the Manning normal-depth calculation
)
...
TRITON_WATERBODIES = (
    True  # acquire NHDPlus HR waterbodies and seed them wet in the init fields + .mann
)
...
TRITON_WATERBODY_LAYER_ID = (
    1  # layer id of "Waterbodies and Areas" (polygons) on the FeatureServer
)
TRITON_WATERBODY_PATH = f"{TRITON_OUT_DIR}/waterbodies.gpkg"  # cached waterbody polygons (auto-acquired by mod21 when missing)
TRITON_WATERBODY_MAX_H = (
    10.0  # cap [m] on the fill-to-rim initial depth inside a waterbody
)
...
TRITON_MAP_HMIN = (
    0.01  # water-depth floor [m] below which velocity is undefined (masked to NODATA)
)
...
TRITON_PERF_SUMMARY = (
    f"{WORKING_DIR}/triton_output/performance.txt"  # final per-rank timing summary
)
TRITON_PERF_DIR = f"{WORKING_DIR}/triton_output/performance"  # per-print-step cumulative timing files (performanceN.txt)
TRITON_PERF_WET_VAR = (
    "H"  # variable used for the wet-cell/volume overlay (must have a cached <var>.nc)
)
