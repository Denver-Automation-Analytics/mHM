...
DOMAIN_NAME = "san_diego"
WORKING_DIR = f"/workspace/data/{DOMAIN_NAME}"  # working directory for all modules
DOMAIN_FILE = f"{WORKING_DIR}/mhm_input/domain/san_diego.shp"  # watershed boundary for all modules
...
GAUGE_DRAINAGE_AREAS_SQMI = {"11023000": 429,}
NODATA = -9999
...
TRITON_COMPARE_TZ = "America/Los_Angeles"  # California tz the TRITON start date (mHM forcing) is expressed in; naive H.nc times are localized to it before converting to UTC for alignment with UTC gauge data
...
TRITON_COMPARE_POINTS = (
    {
        "name": "San Diego R a Fashion Valley at San Diego CA",  # label used in the output filename/plot title
        "lat": 32.7650495526073,  # sampling latitude  [deg, WGS84]
        "lon": -117.16862854169496,  # sampling longitude [deg, WGS84]
        "gauge_id": "11023000",  # USGS site whose stage (00065) is compared
        "gauge_altitude_ft": 16.48,  # altitude of the gauge zero datum [ft]; set to the site's true datum (near the DEM bed at the point)
        "altitude_accuracy_ft": 0.16,  # datum/DEM vertical uncertainty [ft] -> +/- band on the converted depth
    },
)