...
def _verdict(value: Optional[float], low: Optional[float], high: Optional[float]) -> str:
    if value is None:
        return "N/A"
...
def compute_metrics(flux: Dict, inp: Dict) -> List[Metric]:
    ...
    if dS is None:
        residual = None
        closure_rel = None
        closure_note = ("storage states not written; "
    ...
    metrics: List[Metric] = [
        Metric("runoff_coefficient", "Does the runoff coefficient look reasonable?",
               rc_gross, "Q/P (gross)", 0.01, 0.9,
        ...
        Metric("runoff_coefficient_effective",
               "Runoff coefficient vs. mHM effective precip (cross-check).",
               rc_eff, "Q/preEffect", 0.01, 1.0,
               _verdict(rc_eff, 0.01, 1.0),
               f"ΣpreEffect={preEff:.0f} mm"),
        Metric("baseflow_fraction", "Does the baseflow fraction look realistic?",
               bfi, "QB/Q", 0.05, 0.95,
               _verdict(bfi, 0.05, 0.95),
               f"ΣQB={QB:.0f} mm"),
        Metric("aet_pet_ratio", "Does ET look realistic (demand-limited)?",
               aet_pet, "aET/PET", None, 1.0,
               _verdict(aet_pet, None, 1.0),
               f"ΣaET={aET:.0f} mm, ΣPET={PET_f:.0f} mm; aET must not exceed PET"),
        Metric("evaporative_index", "Does ET look realistic (Budyko water limit)?",
               et_index, "aET/P", None, 1.0,
               _verdict(et_index, None, 1.0),
               f"aridity index PET/P={aridity:.2f}" if aridity is not None else ""),
        Metric("recharge_ratio", "Does groundwater recharge (L1_percol) make sense?",
               rech_ratio, "recharge/P", None, 0.6,
               _verdict(rech_ratio, None, 0.6),
               f"Σrecharge={rech:.0f} mm"),
        Metric("recharge_vs_baseflow",
               "Long-term recharge should approximately balance baseflow.",
               rech_bf, "recharge/QB", 0.5, 2.0,
               _verdict(rech_bf, 0.5, 2.0)),
        Metric("water_balance_closure",
               "Overall water balance closure P - aET - Q - ΔS.",
               closure_rel, "residual/P", -0.1, 0.1,
